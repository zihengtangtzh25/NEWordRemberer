...
- 应用版本（APP_VERSION）：v1.0.0
- 备份格式版本（FORMAT_VERSION）：v1.0.0
...
| 导入文件 formatVersion | 当前程序 v1.0.0 | 处理方式 |
|---|---|---|
| 主版本 < 1（如 0.9.x）| v1.0.0 | 黄色警告，尝试按当前 schema 解析，字段缺失的跳过 |
| 主版本 == 1（如 1.0.0 ~ 1.9.9）| v1.0.0 | 绿色，完全兼容，直接导入 |
| 主版本 > 1（如 2.0.0）| v1.0.0 | 红色警告 + confirm："此备份由更高版本导出，不保证新字段完整保留，是否继续？" |
| 缺失 `format` 字段 或 format != 标识 | v1.0.0 | 拒绝，提示"非完整备份文件，可能是今日单词表或背诵结果导出文件" |
