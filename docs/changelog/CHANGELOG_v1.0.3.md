# NEWordRemberer v1.0.3 发布说明（0/8/9 判题交互修正）

- **程序版本**：v1.0.3
- **备份格式版本**：v1.0.0（与 v1.0.0 / v1.0.1 / v1.0.2 完全兼容，无 schema 变化）
- **发布日期**：2026-08-23
- **代码基线**：在 v1.0.2「背诵日历」功能基础上，仅修正 1 处背诵交互延时；零新增字段、零破坏性变更

---

## 0. 一句话概览

> **删除 0/8/9 判题键触发后 500ms 自动跳下一题的延时，把"何时跳下一题"交还给用户——按 Enter 或点击「下一题」按钮。**
...
### 1.2 用户反馈的问题
- 500ms 太短，用户来不及看清反馈颜色 / 看清错题正确答案
- 想要"改判"（按 0/8/9 中的另一个键覆盖结果）也来不及——500ms 一过就自动跳走了
- 与"点击选项按钮"路径（`selectAnswer()`）行为耦合在一起，但实际上 0/8/9 是"口头判题"主交互，体验应当更稳重

### 1.3 设计取舍
- **不**修改 `selectAnswer()`（英译中点选项判题）里的 500ms 自动跳题——那是"鼠标点击"路径，点击时用户已经看完了选项，500ms 跳下一题合理
- **仅**删除 `setResult()`（0/8/9 判题键）里的 500ms 自动跳题——把节奏交还给用户
...
| 步骤 | 旧行为（v1.0.2）| 新行为（v1.0.3）|
|---|---|---|
| 1. 用户按 `0` / `8` / `9` | `setResult('对'/'不熟'/'错')` 被调用 | 同左 |
| 2. 反馈区显示 ✓ / ~ / ✗ 颜色 | ✅ 立即显示 | ✅ 立即显示 |
| 3. 选项按钮高亮（英译中）/ input 禁用（中译英）| ✅ 立即生效 | ✅ 立即生效 |
| 4. 「下一题」按钮显示 | ✅ 立即显示 | ✅ 立即显示 |
| 5. **自动跳下一题** | ⚠️ 500ms 后强制 `nextWord()` | ❌ 不再自动跳 |
| 6. 用户改判（按另一个键） | 500ms 内可改，否则已跳走 | ✅ 任意时间可改（走 `else` 分支，仅刷新 UI + 覆盖最后一条记录） |
| 7. 用户主动跳下一题 | 同左 | ✅ 按 `Enter` 或点击「下一题」按钮 |

### 2.3 与其他判题路径的关系（解耦确认）

| 判题入口 | 触发场景 | 是否 500ms 自动跳题 | v1.0.3 是否改动 |
|---|---|---|---|
| `setResult(result)` | 用户按 **0/8/9** 键 | v1.0.2 是 / v1.0.3 否 | ✅ 删除 |
| `selectAnswer(index)` | 英译中用户点选项按钮 | 仅"答对"时 500ms 自动跳 | ❌ 不动 |
| `checkAnswer()` | 中译英用户按 Enter 提交输入 | 否（用户已显式按 Enter）| ❌ 不动 |
| `Enter` 键（已答题状态）| 用户主动跳下一题 | 是（立即跳，无延时）| ❌ 不动 |

> 三条判题路径在 v1.0.3 中保持各自独立的体验节奏：0/8/9 口头判题 → 用户掌控；点选项判题 → 答对 500ms 后跳；输入答案判题 → 用户按 Enter 提交后再按 Enter 跳。
...
- v1.0.3 不需要新增单元测试（行为变化仅在 UI 交互延时，无新数据格式）
...
4. 改判：按 `0` → 反馈颜色变绿，**不跳题**
5. 按 `Enter` → 跳到下一题 ✅
6. **英译中模式**：点一个选项按钮 → 答对的话 500ms 后**仍自动跳下一题**（这条路径未改） ✅
...
- ✅ `modules/UIManager.js` L407–L408：删除点已加 `[v1.0.3]` 注释
...
| 新增功能 | 0 |
| 删除功能 | 0 |
| 交互行为变化 | 1 处（0/8/9 判题后不再自动跳下一题）|
| 数据格式变化 | 0 |
| 备份格式版本 | 仍为 v1.0.0 |
| localStorage 键数量 | 仍为 4 个（`wordBank` / `todayTask` / `customDate` / `calendarStats`）|
| 兼容性 | 100% 向后兼容 v1.0.0 / v1.0.1 / v1.0.2 |
| 代码改动行数 | -2 / +2（仅 `modules/UIManager.js` 一处）|

> **v1.0.3 是一个最小化的交互修正版本**：仅修正 v1.0.2 中"0/8/9 判题 500ms 自动跳题"带来的体验问题，无任何数据层 / 格式层 / 模块结构变化，升级零风险。
